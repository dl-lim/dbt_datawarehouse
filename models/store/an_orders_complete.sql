...
*/
WITH order_meta AS ( --transpose of db_postmeta
  SELECT * FROM
  public.crosstab(
    $ct$
      SELECT
...
SELECT
	...
	o.tax_total,
	o.customer_id::INT,
	ROUND(o.total_sales::NUMERIC,2) AS total_sales,
  o.date_created::TIMESTAMP, --p_dates
	o.num_items_sold::INT,
	o.shipping_total,
  o.date_created_gmt::TIMESTAMP, --p_dates
	o.returning_customer,
	m.order_key,
  CASE WHEN m.payment_method = 'stripe_cc' THEN 'stripe' ELSE m.payment_method END AS payment_method,
...
/*	CONCAT(
  ...
  EXTRACT(isodow FROM o.date_created::TIMESTAMP) AS week_day,
  EXTRACT(hour FROM o.date_created::TIMESTAMP) AS week_hour,
	CONCAT(
...
transform AS(
  SELECT
    p.*,
    CASE WHEN payment_method = 'stripe' AND stripe_id IS NULL THEN 'test' ELSE 'live' END AS payment_status
  FROM pre_transform p
)

SELECT
  *
FROM transform t
WHERE payment_status = 'live'
